...
def run_dual_proxy_test(concurrency, server_url, test_url, timeout):
    """运行双层代理测试"""
    results = []
    lock = threading.Lock()
    threads = []
    
    print(f"开始双层代理测试，并发数: {concurrency}")
...
def main():
    parser = argparse.ArgumentParser(description="测试双层代理功能")
